...
-----------------------------------------------------
 Farsi support site: http://datalifeengine.ir
-----------------------------------------------------
...
function gregorian_to_jalali ($g_y, $g_m, $g_d) {
	...
	$j_np = jdiv($j_day_no, 12053); /* 12053 = 365*33 + 32/4 */
	$j_day_no = $j_day_no % 12053;

	$jy = 979+33*$j_np+4* jdiv($j_day_no,1461); /* 1461 = 365*4 + 4/4 */
...
function jalali_to_gregorian($j_y, $j_m, $j_d) {
	...
	$gy = 1600 + 400* jdiv($g_day_no, 146097); /* 146097 = 365*400 + 400/4 - 400/100 + 400/400 */
	$g_day_no = $g_day_no % 146097;

	$leap = true;
	if ($g_day_no >= 36525){ /* 36525 = 365*100 + 100/4 */
		$g_day_no--;
		$gy += 100* jdiv($g_day_no, 36524); /* 36524 = 365*100 + 100/4 - 100/100 */
	...
	$gy += 4 * jdiv($g_day_no, 1461); /* 1461 = 365*4 + 4/4 */
	$g_day_no %= 1461;
